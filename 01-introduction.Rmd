...
analysis examples: 
```
&emsp;&emsp;我们要运行的第一个R命令是‘install.packages’。如果你完成了‘swirl’教程，你应该已经学习了这个命令。由于不是每个人都需要所有的包，R默认仅安装基本的包。用户可以根据自己的需要，选择和安装不同功能的包来扩展R的功能。这些包通常都存储在[CRAN](https://cran.r-project.org/)，而且已经被审查过是否有常见的错误，是否有一个专门的维护者。如果你知道你要安装的包的名字，那么安装将非常简单。你只需要在R里运行‘install.packages’命令即可。比如‘rafalib’包的安装，这个包
...
&emsp;&emsp;从现在开始，你会发现我们有时直接加载包而不需要安装它们。这是因为一旦你安装了包，
