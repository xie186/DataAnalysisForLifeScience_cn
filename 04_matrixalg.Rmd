...
```
&emsp;&emsp;在R中，我们可以通过`lm`函数来符合这个模型。我们将在接下来详细的描述这个函数，但是这里给出了一个预览：
```{r}
tt2 <-tt^2
fit <- lm(y~tt+tt2)
summary(fit)$coef
...
```
&emsp;&emsp;线性代数符号实际上是数学描述和操作线性模型的简化形式，就像R的代码。我们将讨论这个符号的基本知识并且使用R来展示一些例子。整个练习的要点是展示如何使用矩阵符号写出上面的几个模型，并且解释它是如何有效来解最小二乘方程的。我们从简单定义符号和矩阵乘法开始，但是请忍受我们，因为我们最终还是回到了实际应用。
```{block2 echo=FALSE}
...
```
&emsp;&emsp;在下落物体、父亲和儿子升高以及老鼠体重的例子中，与这些数据相关的随机变量我们用$Y_1,\dots,Y_n$来表示。我们可以认为这些数据为一个向量。实际上，在R中我们已经做了相关的工作。
```{r,message=FALSE}
data(father.son,package="UsingR")
...
```
&emsp;&emsp;这个矩阵的维度是$N \times 2$。我们可以用R的方式来创建这个矩阵：
```{r}
n <- 25
tt <- seq(0,3.4,len=n) ##time in secs, t is a base function
...
```
&emsp;&emsp;举个例子，我们将展示在R中如何使用`matrix` 而不是 `cbind`来创建矩阵：
```{r}
N <- 100; p <- 5
X <- matrix(1:(N*p),N,p)
head(X)
...
```
&emsp;&emsp;默认的，在R中矩阵是按列填充的。参数`byrow=TRUE`能够让我们以按行填充的方式来创建矩阵。
```{r}
N <- 100; p <- 5
...
```
&emsp;&emsp;矩阵$X$的逆矩阵用$X^{-1}$来表示，当相乘的时候，它有$X^{-1}X=I$相同的属性。当然，不是所有的矩阵都有逆矩阵。例如，当一个$2\times 2$ 的矩阵与一个数相乘时得到的所有的条目都没有逆矩阵。正如我们将看到的，我们到了关于线性模型的应用部分，能够计算矩阵的逆矩阵是非常有用的。R的一个非常便利的方面是它包含了一个预定义的函数`solve`来解决这个问题。下面我们就将演示如何用它来解线性方程。
```{r}
X <- matrix(c(1,3,2,1,-2,1,1,1,-1),3,3)
...
```
&emsp;&emsp;现在我们已经准备好了，看看矩阵代数在分析数据时是如何使用的。我们从一些简单的例子开始，然后最终达到主要的目的：如何用矩阵代数的符号写出线性模型并能够解决最小二乘问题。
...
```
&emsp;&emsp;为了计算样本的平均值和方差，我们可以使用以下公式：$\bar{Y}=\frac{1}{N} Y_i$和$\mbox{var}(Y)=\frac{1}{N} \sum_{i=1}^N (Y_i - \bar{Y})^2$。我们可以用矩阵乘法来表示。首先，我们定义$N \times 1$的矩阵为1s：
$$
A=\begin{pmatrix}
1\\
1\\
...
```
请注意，我们要乘以标量$1/N$。在R中，我们用`%*%`来计算矩阵的乘法。
```{r,message=FALSE}
data(father.son,package="UsingR")
y <- father.son$sheight
print(mean(y))
...
```
&emsp;&emsp;我们稍后将会看到，矩阵的转置乘以另一个矩阵在统计学中是很常见的。事实上，这是在R中很常见的一个函数。
```{r}
...
```
在R中，你只需将一个矩阵代入函数`crossprod`中，它将会执行：$r^\top r$，因此我们只需简单地输入：
```{r}
r <- y - barY
crossprod(r)/N
```
...
Now we are ready to put all this to use. Let's start with Galton's example. If we define these matrices:
 ```
&emsp;&emsp;现在我们准备把之前的所学的知识都用上。让我们从高尔顿的例子开始。如果我们定义这些矩阵为：
$$
\mathbf{Y} = \begin{pmatrix}
Y_1\\
Y_2\\
...
```
因此，现在我们已经准备好来确定哪一个值是$/beta$的最小值，我们可以通过使用微积分的方法来求最小值。
```{block2 echo=FALSE}
...
```
&emsp;&emsp;现在有一系列规则允许我们用矩阵符号来计算偏微分方程。首先通过将导数等于0，然后在求解$\beta$，我们有我们的解决办法。这里我们唯一需要知道的是上面方程的导数是：
$$
...
```
我们有我们的解题方法。我们通常在$\beta$上面加上一个帽子即$\hat{\beta}$来解决这个问题，因为它是在我们对真实$\beta$值估计时产生的数据。
```{block2 echo=FALSE}
...
```
请记住，最小二乘就像一个正方形（它本身可以乘以任何数据），这个公式类似于导数$f(x)^2$ being $2f(x)f\prime (x)$。
```{block2 echo=FALSE}
#### Finding LSE in R
```
#### 在R中使用最小二乘估计
...
```
现在，对于任何一个$x$值，我们都可以通过计算的估计来得到相应的结果。
...
```
 公式$\hat{\boldsymbol{\beta}}=(\mathbf{X}^\top \mathbf{X})^{-1} \mathbf{X}^\top \mathbf{Y}$是我们在数据分析过程中使用最广泛的结果之一。这种方法的优点之一就是我们可以在许多不同的情况下使用它。例如：在我们高空坠落物体的问题中：
 ```{r}
set.seed(1)
g <- 9.8 #meters per second
n <- 25
...
```
比萨斜塔的高为56米。因为我们只是把物体丢了下去，所以它是没有初始速度的，以及重力加速度的一半是9.8/2=4.9米每平方秒。
```{block2 echo=FALSE}
#### The `lm` Function
```
#### `lm`函数
...
```
R有一个非常方便的函数可以适合这些模型。稍后我们将进一步了解这个函数，这里有一个预览：
```{r}
X <- cbind(tt,tt^2)
fit=lm(y~X)
summary(fit)
...
```
&emsp;&emsp;上面我们已经展示了如何用线性代数来写线性模型。我们也举几个例子来说明这个方法，其中许多是与设计实验有关的例子。我们还演示了如何获得最小二乘估计。然而，其中很重要的一点是：因为 $y$是一个随机变量，因此这些估计也是随机的。在接下来的章节中，我们将学习如何计算这些估计的标准误差并且用它来进行相应的推导。
